...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="correos">
        <xsl:for-each select="carta">
            <xsl:sort select="fechaExpedicion/@dia" order="descending"/>
            <xsl:sort select="fechaExpedicion/@mes" order="descending"/>
            <xsl:sort select="fechaExpedicion/@anyo" order="descending"/>
